...
class FusionTransformer(nn.Module):
    ...
    def __init__(
            ...
            dropout=0.1,
            max_len=10,
            use_wide_branch=False  # 新增：是否使用 Wide 分支
    ):
        super().__init__()
...
class SWENet(nn.Module):
    ...
    输入:
        ...
        point_feats: (B, C_point)           # 点特征（18维，不含降水累积和产品值）
    输出:
        y_pred: (B,) 标准化后的SWE值
    """
    def __init__(
...
# ============= 模型工厂函数 =============
def create_model(model_type, use_wide_branch=False, **kwargs):
    """
    根据类型创建模型
