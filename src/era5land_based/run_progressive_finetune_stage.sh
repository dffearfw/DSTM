...
# 当前main_tune的站点十折会给所有可训练参数统一使用fine_tune_lr。
# 因此这里为不同冻结策略固定一个“策略级学习率”。
# 同一策略在M0-M4中必须完全一致。
...
print("✅ 固定数据清单检查通过")
print(f"   内部CV池:     {counts['internal_cv_samples']:,}")
print(f"   内部留出测试: {counts['internal_test_samples']:,}")
...
REFERENCE_SPLIT_JSON="${MANIFEST_DIR}/station_cv_fold_splits_seed43_reference.json"
...
for STRATEGY in "${STRATEGY_LIST[@]}"; do
    ...
    echo "策略级学习率:      ${FT_LR}"
    echo "内部评估:          平衡站点10折；每折训练后立即测该折留出站点"
    echo "旧固定1000条:      不作为正式内部主结果"
    ...
    REUSE_CV=0
    if [[ "${#EXISTING_FOLD_MODELS[@]}" -eq 10 ]]; then
        ...
        if [[ -f "${EXISTING_RUN_DIR}/station_cv10_oof_predictions.csv" &&
            ...
            if python - "${EXISTING_RUN_DIR}/cv_station_level_aggregated_results.json" <<'PY_CHECK'
import json, sys
path = sys.argv[1]
...
assert data.get("evaluation_policy", {}).get("internal") == "immediate_heldout_fold_oof"
print("✅ 已有结果符合平衡站点OOF协议")
PY_CHECK
            then
                REUSE_CV=1
            fi
...
payload = {
    ...
    "internal_test_definition": (
        "Deterministic balanced station-level 10-fold OOF; the old fixed "
        "1000-sample holdout is not the formal internal primary result."
    ),
