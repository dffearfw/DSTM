...
# 0=旧版完整Fusion解冻；正整数N=仅解冻顶部N个Transformer block。
# 本次受控实验请显式设置 FUSION_FT_MAX_UNFROZEN_LAYERS=2。
...
python - \
    ...
    "${FUSION_FT_PATIENCE}" <<'PY'
import math
import sys

head_lr = float(sys.argv[1])
transformer_lr = float(sys.argv[2])
integer_values = [int(value) for value in sys.argv[3:]]
(
    warmup,
    interval,
    max_unfrozen_layers,
    ...
    if [[ "${INNER_PILOT_ONLY}" == "0" &&
        ...
        if [[ -f "${EXISTING_RUN_DIR}/station_cv10_oof_predictions.csv" &&
            ...
            if python - \
                ...
                "${INCLUDE_FIXED_INTERNAL_IN_CV}" <<'PY_CHECK'
import json, sys
path = sys.argv[1]
strategy = sys.argv[2]
...
assert policy.get("checkpoint_gate") == "frozen_relative_non_degradation"
...
print("✅ 已有结果符合nested OOF + Frozen-relative综合选模协议")
PY_CHECK
            then
                REUSE_CV=1
            fi
...
python - \
    ...
    "${EXTERNAL_DATA}" \
    "${NORMALIZATION_CONFIG}" \
    "${FUSION_FT_MAX_UNFROZEN_LAYERS}" <<'PY'
import json
import sys
