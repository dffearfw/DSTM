...
class OnlineERASWEDataset(Dataset):
    ...
    def _compute_minmax(self):
        ...
        dyn_min = np.nanmin(dyn_arr)
        dyn_max = np.nanmax(dyn_arr)

        # 避免除零
        if dyn_max - dyn_min < 1e-6:
            dyn_max = dyn_min + 1.0

        self.dyn_min = dyn_min
        self.dyn_max = dyn_max

        print(f"  动态变量 min={dyn_min:.4f}, max={dyn_max:.4f}")

        # 2) 静态变量（全零，标准化后仍为零）
        # 由于静态变量是占位符，我们设置min=0, max=1
        self.static_min = 0.0
        self.static_max = 1.0
        ...
        # 收集所有SWE值
        swe_values = []
        for swe_grid in self.era5_grids.values():
            valid_values = swe_grid[np.isfinite(swe_grid)]
            swe_values.extend(valid_values.tolist())

        if swe_values:
            swe_min = np.nanmin(swe_values)
            swe_max = np.nanmax(swe_values)

            if swe_max - swe_min < 1e-6:
                swe_max = swe_min + 1.0
        else:
            swe_min, swe_max = 0.0, 1.0

        self.swe_min = swe_min
        self.swe_max = swe_max

        print(f"  SWE min={swe_min:.4f}, max={swe_max:.4f}")
