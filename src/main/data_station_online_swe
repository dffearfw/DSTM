...
LABEL_ROOT = Path(r"/root/autodl-tmp/ablation/fusedswe/cn")  # 用于获取参考网格

# 站点SWE数据路径
STATION_SWE_CSV = Path(r"/root/autodl-tmp/ablation/station_swe_data.xlsx")
...
def conv_static_path(var: str, year: int, threshold: float = 0.5) -> List[Path]:
    ...
    if var == "clamday":
        ...
        pattern = f"*{year}*threshold{threshold}*_masked.tif"
        ...
        if not files:
            for thresh in [0.5, 1.0]:
                if thresh != threshold:
                    pattern = f"*{year}*threshold{thresh}*_masked.tif"
                    files = list(clamday_path.glob(pattern))
                    if files:
                        print(f"    使用阈值 {thresh} 代替 {threshold}")
                        break
        ...
        if not files:
            files = list(clamday_path.glob("*CalmDays*.tif"))
        ...
        valid_files = [f for f in dem_files if f is not None]
        print(f"    DEM文件: 找到 {len(valid_files)} 个有效文件")
        return valid_files
...
class StationSWEDataset(Dataset):
    ...
    def _load_fused_swe_labels(self):
        """加载FusedSWE标签数据 - 只加载目标年份"""
        print(f"\n加载FusedSWE标签数据...")

        self.label_data = {}

        fused_swe_root = Path(r"/root/autodl-tmp/ablation/fusedswe/cn")
        if not fused_swe_root.exists():
            print(f"  ⚠ FusedSWE目录不存在: {fused_swe_root}")
            return
        ...
        # 获取所有年份的文件
        label_files = sorted(list(fused_swe_root.glob("*.tif")))
        print(f"  找到 {len(label_files)} 个FusedSWE文件")

        if not label_files:
            print(f"  ⚠ 没有找到FusedSWE文件")
            return
        ...
        for label_file in label_files:
            try:
                ...
                self.label_data[date_match] = (label_arr, label_nodata)
                loaded_count += 1
                year_count[date_match.year] = year_count.get(date_match.year, 0) + 1

                if loaded_count <= 5:
                    print(f"  ✓ {date_match.strftime('%Y-%m-%d')}: 加载成功")

            except Exception as e:
                print(f"  ✗ 加载失败 {label_file.name}: {e}")
                continue

        print(f"\n  ✅ 共加载 {loaded_count} 个日期的FusedSWE数据")

        if loaded_count > 0:
            dates = sorted(self.label_data.keys())
    ...
    def _load_all_features(self):
        ...
        print(f"1. 加载chelsa_sfxwind作为时间基准...")
        # 🔥 只传递 var，不传递 self.load_years
        chelsa_data, chelsa_dates = self._load_single_variable("chelsa_sfxwind")
        if chelsa_data is not None:
            self.conv_dyn_data["chelsa_sfxwind"] = chelsa_data
            self.all_dates = chelsa_dates
            self.date_to_index = {d: i for i, d in enumerate(chelsa_dates)}
            print(f"  chelsa_sfxwind: {len(chelsa_dates)} 个日期")

        for var in ["lst", "rh", "pr"]:
            print(f"\n2. 加载 {var}...")
            # 🔥 只传递 var
            var_data, var_dates = self._load_single_variable(var)
            if var_data is not None:
                aligned_data = self._align_to_timeline(var, var_data, var_dates)
                self.conv_dyn_data[var] = aligned_data
                print(f"  {var}: {var_data.shape[0]}天 -> {aligned_data.shape[0]}天 (对齐后)")

        print(f"\n统一时间轴: {len(self.all_dates)} 个日期")
        print(f"日期范围: {self.all_dates[0].strftime('%Y-%m-%d')} 到 {self.all_dates[-1].strftime('%Y-%m-%d')}")

        self._load_static_conv_features()
        self._load_point_features()
    
    
    def _align_to_timeline(self, var: str, var_data, var_dates):
        """
        将数据对齐到统一时间轴 (self.all_dates)
        """
    ...
    def _load_single_variable(self, var: str):
        ...
        for year in target_years:
            ...
            if var == "chelsa_sfxwind":
                ...
                pattern = f"ERA5_ST_{year}*.tif"
            elif var == "rh":
                pattern = f"ERA5_RH_DailyMean_{year}_*.tif"
            elif var == "pr":
                pattern = f"*{year}*.tif"
            else:
                pattern = "*.tif"
        ...
        try:
            ...
            if np.any(nan_mask):
                if np.sum(~nan_mask) > 0:
                    ...
                    nearest_rows, nearest_cols = nearest_indices
                    aligned[nan_mask] = aligned[nearest_rows[nan_mask], nearest_cols[nan_mask]]
                else:
        ...
        if clamday_files:
            with rasterio.open(clamday_files[0]) as ds:
                ...
                if nodata is not None:
                    clamday_data_raw[clamday_data_raw == nodata] = np.nan
                clamday_data_raw[(clamday_data_raw < 0) | (clamday_data_raw > 1)] = np.nan

            self.clamday_data = self._align_single_layer(clamday_data_raw, clamday_files[0])

            # IDW 插值填充 NaN
            nan_mask = np.isnan(self.clamday_data)
            if np.any(nan_mask):
                self.clamday_data = self._idw_interpolate(self.clamday_data, nan_mask)

            print(f"  Clamday形状: {self.clamday_data.shape}")
            print(f"  Clamday范围: [{self.clamday_data.min():.4f}, {self.clamday_data.max():.4f}]")
        else:
            self.clamday_data = np.zeros((self.H, self.W), dtype=np.float32)
            print(f"  警告: 未找到Clamday文件")

        # 2. 加载 DEM（读取所有波段 - 与预训练一致）
        dem_files = conv_static_path("dem", self.year_target)
        self.dem_data = []  # 重置为空列表
        ...
        if dem_files:
            ...
            for dem_file in dem_files:
                with rasterio.open(dem_file) as ds:
                    # 🔥 关键：读取所有波段
                    dem_all_bands = ds.read().astype(np.float32)
                    ...
                    for band_idx in range(n_bands):
                        ...
                        if np.any(invalid_mask):
                            print(f"      DEM波段{band_idx+1}: 发现 {np.sum(invalid_mask)} 个无效值")
                            band_data[invalid_mask] = np.nan

                        # 地理对齐
                        aligned_band = self._align_single_layer(band_data, dem_file)

                        # IDW 插值填充 NaN
                        nan_mask = np.isnan(aligned_band)
                        if np.any(nan_mask):
                            aligned_band = self._idw_interpolate(aligned_band, nan_mask, power=2, radius=10)

                            # 二次保险
                            if np.any(np.isnan(aligned_band)):
                                mean_val = np.nanmean(aligned_band)
                                if np.isnan(mean_val):
                                    mean_val = 0.0
                                aligned_band = np.nan_to_num(aligned_band, nan=mean_val)

                        self.dem_data.append(aligned_band)

                        # 获取波段描述
                        band_desc = ds.descriptions[band_idx] if ds.descriptions and band_idx < len(ds.descriptions) else f"band_{band_idx+1}"
                        print(f"      DEM波段{len(self.dem_data)}: {band_desc} 形状: {aligned_band.shape}, "
                              f"范围: [{aligned_band.min():.2f}, {aligned_band.max():.2f}]")
        else:
            # 如果没有DEM文件，创建一个默认波段
    ...
    def _load_point_features(self):
        ...
        if isinstance(self.year_target, list):
            years = self.year_target
        else:
            years = [self.year_target]

        ls_data_loaded = False
        all_ls_bands = []  # 🔥 存储所有年份的波段

        for year in years:
            ls_file = point_var_path("ls", year)
            if ls_file and ls_file.exists():
                print(f"    找到LS文件: {ls_file.name} (年份: {year})")
                try:
                    with rasterio.open(ls_file) as ds:
                        ls_data_raw = ds.read()  # (n_bands, H, W)
                        src_transform = ds.transform
                    ...
                    for i in range(ls_data_raw.shape[0]):
                        ...
                        all_ls_bands.append(band_aligned)
                    print(f"      已添加 {ls_data_raw.shape[0]} 个波段")
                    ls_data_loaded = True
                except Exception as e:
                    print(f"      处理LS文件失败: {e}")
                    continue
        ...
        if ls_data_loaded:
            self.ls_data = np.stack(all_ls_bands, axis=0)
            print(f"      LS数据总形状: {self.ls_data.shape} (波段数: {self.ls_data.shape[0]})")
        else:
            self.ls_data = np.zeros((6, self.H, self.W), dtype=np.float32)
            print(f"  警告: 未找到LS文件，使用6个默认波段 (全0)")

        if ls_data_loaded:
            print(f"\n  预计算光谱解混丰度...")
            self._precompute_unmixing()
        else:
            self.abundance_maps = np.zeros((5, self.H, self.W), dtype=np.float32)
            self.abundance_maps[0, :, :] = 1.0
            print(f"  警告: 无LS数据，使用默认丰度图")
        ...
        print(f"\n  【点特征加载统计】")
        print(f"    LS数据: {'✓' if ls_data_loaded else '✗'} (形状: {self.ls_data.shape})")
        print(f"    哨兵1数据: {len(self.all_s1_dates)} 个日期")
        if len(self.all_s1_dates) > 0:
    ...
    def _load_sentinel1_data(self):
        ...
        all_s1_files = []
        for year in years:
            s1_files = point_var_path("S1_VV", year)
            if s1_files:
                if isinstance(s1_files, list):
                    all_s1_files.extend(s1_files)
                else:
                    all_s1_files.append(s1_files)

        if not all_s1_files:
            print(f"    ⚠ 警告: 未找到哨兵1数据")
            return

        for s1_file in all_s1_files:
            try:
                with rasterio.open(s1_file) as ds:
                    ...
                    for band_idx in range(1, n_bands + 1):
                        ...
                        aligned_data = self._align_single_layer(band_data, s1_file)
        ...
        if self.all_s1_dates:
            print(f"\n    【总结】哨兵1数据: {len(self.all_s1_dates)} 个日期")
        else:
            print(f"    ⚠ 警告: 没有成功加载任何哨兵1数据")
                    ...
                    for band_idx in range(1, n_bands + 1):
                        ...
                        if date_match:
                            ...
                            if not date_match:
                                continue
                            y, m, d = int(date_match.group(1)), int(date_match.group(2)), int(date_match.group(3))
                            pol = 'V' if 'V' in desc_clean.upper() else 'H' if 'H' in desc_clean.upper() else None
                        ...
                        band_data = ds.read(band_idx).astype(np.float32)
                        aligned_data = self._align_single_layer(band_data, smap_file)
    ...
    def _load_station_data(self, station_csv: Path):
        ...
        for idx, row in aggregated_df.iterrows():
            ...
            self.meta_index.append({
                'feature_date': feature_date,      # 用于提取特征
    ...
    def _build_point_features_station(self, date_dt: datetime, r: int, c: int) -> np.ndarray:
        ...
        if len(point_features) != 21:
            reason = f"维度错误: 实际{len(point_features)} != 21"
    ...
    def _compute_minmax(self):
        ...
        if self.clamday_data is not None:
            valid_data = self.clamday_data[np.isfinite(self.clamday_data)]
            if len(valid_data) > 0:
        ...
        for i, dem_band in enumerate(self.dem_data):
            if dem_band is not None:
                valid_data = dem_band[np.isfinite(dem_band)]
                if len(valid_data) > 0:
                    conv_mins.append(float(np.min(valid_data)))
                    conv_maxs.append(float(np.max(valid_data)))
                else:
        ...
        # 2.1 LS特征 (6个)
        if hasattr(self, 'ls_data') and self.ls_data is not None:
            num_ls_bands = min(6, self.ls_data.shape[0])
        ...
        for meta in self.meta_index:
            ...
            if date_idx is not None and "pr" in self.conv_dyn_data and "lst" in self.conv_dyn_data:
                ...
                for i in range(valid_days):
                    ...
                    if np.isfinite(pr_val) and pr_val != INVALID_PR and np.isfinite(temp_val) and temp_val != INVALID_LST:
                        cum_pr += float(pr_val)
                        if temp_val < 1.0:
        ...
        # 2.7 原产品值 (FusedSWE) - 第21维
        print(f"\n  原产品值 (FusedSWE):")
        product_values = []

        for meta in self.meta_index:
            date_dt = meta['feature_date']  # 🔥 使用 feature_date
            r = meta['row']
            c = meta['col']
            if date_dt in self.label_data:
                label_arr, label_nodata = self.label_data[date_dt]
        ...
        # 预期维度: LS(6) + S1(5) + SMAP_TB(2) + SMAP_mask(2) + 经纬度(2) + DOY(1) + 降水(2) + 产品(1) = 21
    ...
    def _get_alternative_microwave_value(self, date: datetime, r: int, c: int, band: str):
        ...
        for data_date, data_dict in data_source.items():
            if band_key in data_dict:
                ...
                valid_values = layer[layer != nodata]
    ...
    def _get_spatial_nearest_value(self, date: datetime, r: int, c: int, band: str):
        ...
        valid_values = data_layer[data_layer != nodata]
        if len(valid_values) > 0:
    ...
    def _build_transformed_physical_feature(self, y_fused, lst, pr_30d, r, c):
        """
        显式物理公式转换：将 FusedSWE 转换为热力稳定性指数 (SHTSI)

        参数:
            y_fused: 原始 FusedSWE 产品值 (mm)
            lst: 地表温度 (摄氏度)
            pr_30d: 过去30天总降水 (mm)
            r, c: 行列坐标，用于获取DEM高程
        ...
        返回:
            转换后的物理特征值 (归一化前)
        """
        # 1. 基础热力负载 (LST 转为正值，负温时积雪稳定)
        t_plus = max(0, lst) 

        # 2. 地形增益 (利用 DEM 计算简易地形指数)
        # 获取 DEM 值（第一个波段通常是高程）
        dem_val = float(self.dem_data[0][r, c]) if len(self.dem_data) > 0 else 0.0
        # 高海拔通常积雪更稳，低海拔融化更快
        # 归一化地形因子：海拔每增加1000m，因子增加0.2
        topo_factor = 1.0 + (dem_val / 5000.0)  # 5000m 时 factor=2.0

        # 3. 水文缓冲 (使用30天累积降水)
        # 降水越多，土壤湿度越大，热容越高，融化越慢
        hydro_buffer = np.log1p(pr_30d)  # ln(1 + pr)

        # 4. 组合转换公式 (SHTSI - Snow Heat Stability Index)
        # k 是衰减敏感度，k越大，温度对积雪的破坏力越明显
        k = 0.05 

        # 计算衰减因子
        # 温度越高，地形越低，降水越少 → 衰减越大 → 输出越小
        denominator = topo_factor * (1.0 + hydro_buffer)
        if denominator > 0:
            decay = np.exp(-k * (t_plus / denominator))
        else:
            decay = 0.0

        # 5. 输出转换后的物理特征
        # 原始产品值 × 衰减因子 = 热力稳定性修正后的积雪
        result = y_fused * decay

        # 添加微小偏移防止除零
        if np.isnan(result) or np.isinf(result):
            result = y_fused  # 兜底：返回原始值
    ...
    def __getitem__(self, idx: int):
        ...
        for retry in range(max_retry):
            ...
            try:
                ...
                conv_t = (conv_t - c_min) / (c_max - c_min + eps)
                conv_t = torch.clamp(conv_t, 0.0, 1.0)
...
class MixedFineTuneDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if idx < len(self.station_dataset):
            ...
            # ============ 预训练数据仓库：需要重新计算第21维为 SHTSI ============
            p_idx = self.selected_pretrain[idx - len(self.station_dataset)]
            raw_res = self.pretrain_dataset[p_idx]

            # 获取原始数据（预训练数据的第21维是0）
            if len(raw_res) >= 4:
                res_conv, res_point_20d, res_target, res_mask = raw_res[:4]
            else:
                res_conv, res_point_20d, res_target = raw_res[:3]
                res_mask = torch.where(res_target > 0, torch.ones_like(res_target), torch.zeros_like(res_target))

            # 🔥 关键修复：为预训练样本计算 SHTSI（第21维）
            # 获取该样本的元数据
            date_dt, r, c = self.pretrain_dataset.meta_index[p_idx][:3]

            # 计算 SHTSI
            sh_21 = self._compute_shotsi_for_pretrain_sample(
                date_dt, r, c, res_target, res_conv
            )

            # 构建完整的21维点特征：前20维 + SHTSI
            if isinstance(res_point_20d, torch.Tensor):
                res_point_20d_flat = res_point_20d.flatten()
                # 确保前20维存在
                if res_point_20d_flat.shape[0] >= 20:
                    point_20d = res_point_20d_flat[:20]
                else:
                    # 如果不足20维，补0
                    point_20d = torch.cat([res_point_20d_flat, torch.zeros(20 - res_point_20d_flat.shape[0])])
            else:
                point_20d = torch.zeros(20)

            # 拼接前20维 + SHTSI（第21维）
            sh_21_tensor = torch.tensor([sh_21], dtype=torch.float32)
            res_point = torch.cat([point_20d, sh_21_tensor])
        ...
        # 🔥 点特征: 确保是21维
        res_point = torch.as_tensor(res_point, dtype=torch.float32)
        if res_point.dim() == 0:
            res_point = res_point.unsqueeze(0)
        if res_point.shape[0] < 21:
            padding = torch.zeros(21 - res_point.shape[0])
            res_point = torch.cat([res_point, padding])
        elif res_point.shape[0] > 21:
            res_point = res_point[:21]
    ...
    def _compute_shotsi_for_pretrain_sample(self, date_dt, r, c, target_norm, conv_feats=None):
        """
        为预训练样本计算 SHTSI（热力稳定性指数）

        使用与 StationSWEDataset 完全相同的物理公式：
        SHTSI = y_fused × exp(-k × (LST⁺ / (地形因子 × (1 + ln(1+降水)))))

        Args:
            date_dt: 日期
            r, c: 行列坐标
            target_norm: 归一化的 FusedSWE 目标值
            conv_feats: 可选的卷积特征（用于获取环境数据）

        Returns:
            sh_21: 归一化的 SHTSI 值 (0-1)
        """
        try:
            # 1. 反归一化获取原始 FusedSWE 值
            ...
            # 2.1 LST（地表温度，摄氏度）
            lst_raw = 0.0
            if hasattr(self.pretrain_dataset, 'conv_dyn_data') and "lst" in self.pretrain_dataset.conv_dyn_data:
                date_idx = self.pretrain_dataset.date_to_index.get(date_dt)
                if date_idx is not None:
                    lst_raw = float(self.pretrain_dataset.conv_dyn_data["lst"][date_idx, r, c])
                    if lst_raw == -9999.0 or not np.isfinite(lst_raw):
                        lst_raw = 0.0

            # 2.2 30天累积降水（与第18维保持一致）
            cum_pr_30d = 0.0
            if hasattr(self.pretrain_dataset, 'conv_dyn_data') and "pr" in self.pretrain_dataset.conv_dyn_data:
                date_idx = self.pretrain_dataset.date_to_index.get(date_dt)
                if date_idx is not None:
                    start_idx = max(0, date_idx - 30)
                    pr_history = self.pretrain_dataset.conv_dyn_data["pr"][start_idx:date_idx + 1, r, c]
                    for p in pr_history:
                        if p != -9999.0 and np.isfinite(p):
                            cum_pr_30d += float(p)

            # 2.3 DEM（数字高程模型）
            dem_val = 0.0
            if hasattr(self.pretrain_dataset, 'dem_data') and len(self.pretrain_dataset.dem_data) > 0:
                dem_val = float(self.pretrain_dataset.dem_data[0][r, c])
                if not np.isfinite(dem_val):
                    dem_val = 0.0

            # 3. 物理公式计算 SHTSI
            # 3.1 基础热力负载
            t_plus = max(0, lst_raw)

            # 3.2 地形因子（海拔每增加1000m，因子增加0.2）
            topo_factor = 1.0 + (dem_val / 5000.0)

            # 3.3 水文缓冲
            hydro_buffer = np.log1p(cum_pr_30d)
