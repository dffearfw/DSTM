...
# ============ 运行模式配置（frozen 用 evaluate，其他用 fine_tune） ============
declare -A MODE=(
    ["frozen"]="evaluate"
    ["last_layer_only"]="fine_tune"
    ["fusion_ft"]="fine_tune"
    ["point_ft"]="fine_tune"
    ["spatial_ft"]="fine_tune"
    ["partial"]="fine_tune"
    ["none"]="fine_tune"
)
...
if [ -n "${RESUME_DIR}" ] && [ -d "${RESUME_DIR}" ]; then
    RUN_ROOT_DIR="${RESUME_DIR}"
    echo "🔄 续跑模式: 使用已有目录 ${RUN_ROOT_DIR}"
elif [ -n "${RESUME_DIR}" ] && [ ! -d "${RESUME_DIR}" ]; then
    echo "❌ 错误: 续跑目录不存在: ${RESUME_DIR}"
    exit 1
else
    RUN_ROOT_DIR="${EXPERIMENTS_BASE_DIR}/run_${MAIN_TIMESTAMP}"
    mkdir -p ${RUN_ROOT_DIR}
    echo "🆕 新实验: ${RUN_ROOT_DIR}"
fi
...
is_completed() {
    local seed=$1
    local strategy=$2
    local exp_dir="${RUN_ROOT_DIR}/seed_${seed}/${strategy}"
    ...
    # 检查是否有 cv_station_level_aggregated_results.json
    if [ -f "${exp_dir}/cv_station_level_aggregated_results.json" ]; then
        return 0
    fi
    
    # 检查是否有 best_model.pth 或 final_model.pth
    if [ -f "${exp_dir}/best_model.pth" ] || [ -f "${exp_dir}/final_model.pth" ]; then
        return 0
    fi
    
    # 🔥 检查子目录下是否有 fine_tune_scatter_chinese_fold_*.png（说明至少跑了一部分）
    local fold_png=$(find "${exp_dir}" -maxdepth 3 -type f -name "fine_tune_scatter_chinese_fold_*.png" 2>/dev/null | head -1)
    if [ -n "${fold_png}" ]; then
        # 检查是否有 config.json（说明确实开始训练了）
        local config_file=$(find "${exp_dir}" -maxdepth 3 -type f -name "config.json" 2>/dev/null | head -1)
        if [ -n "${config_file}" ]; then
            return 0  # 有 config.json 且有散点图 → 认为已完成（至少跑了一部分）
        fi
    fi
...
if [ "${RESUME_MODE}" = true ]; then
    ...
    for SEED in "${SEEDS[@]}"; do
        for STRATEGY in "${STRATEGIES[@]}"; do
            TOTAL=$((TOTAL + 1))
            if is_completed ${SEED} ${STRATEGY}; then
    ...
    if [ ${PENDING} -eq 0 ]; then
        echo ""
        echo "✅ 所有任务已完成！跳过运行，直接进入绘图。"
        goto_plot=true
    else
        echo "   继续运行 ${PENDING} 个未完成的任务..."
        goto_plot=false
    fi
else
    goto_plot=false
...
for SEED in "${SEEDS[@]}"; do
    ...
    # 初始化日志文件（如果续跑且文件存在，追加而不是覆盖）
    if [ -f "${LOG_FILE}" ] && [ "${RESUME_MODE}" = true ]; then
        echo "   📝 追加到已有日志文件"
        {
            echo ""
            echo "=========================================="
    ...
    for idx in "${!STRATEGIES[@]}"; do
        ...
        if [ "${RESUME_MODE}" = true ]; then
            if is_completed ${SEED} ${STRATEGY}; then
                echo ""
                echo "=========================================="
                echo -e "${GREEN}⏭️ [SEED=${SEED}] 策略 ${STRATEGY} 已完成，跳过${NC}"
                echo "=========================================="
                continue
            fi
        fi
        ...
        # 🔥 构建命令 - 根据 MODE 决定
        if [ "${MODE[$STRATEGY]}" = "evaluate" ]; then
            # frozen 策略：只评估，不训练
        ...
        # 第一个策略强制重新计算划分（但如果缓存已存在则不强制）
        if [ "$FIRST_STRATEGY" = true ] && [ ! -f "${SPLIT_CACHE_FILE}" ]; then
            CMD="${CMD} --force_recompute_split"
            echo "   🔥 缓存不存在，强制重新计算划分 (seed=${SEED})"
            FIRST_STRATEGY=false
        else
            if [ -f "${SPLIT_CACHE_FILE}" ]; then
                echo "   📦 复用已有划分缓存"
            else
                echo "   ⚠️ 缓存不存在，将自动创建"
            fi
...
strategy_order = ['last_layer_only', 'fusion_ft', 'point_ft', 'spatial_ft', 'partial', 'none']
