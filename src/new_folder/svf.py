...
def export_zero_misclassifications(df):
    ...
    # 导出为 CSV 文件（包含经纬度）
    csv_path = OUTPUT_DIR / 'zero_misclassifications.csv'
    
    # 选择要导出的列
    export_columns = [
        'date', 'station_id', 'longitude', 'latitude', 
        'swe', 'fused_swe', 'error', 'abs_error'
    ]
    
    # 确保所有列都存在
    available_columns = [col for col in export_columns if col in zero_misclass.columns]
    zero_misclass[available_columns].to_csv(csv_path, index=False, encoding='utf-8-sig')
    ...
    # 可选：导出为 Shapefile 兼容的格式（如果需要空间分析）
    shp_csv_path = OUTPUT_DIR / 'zero_misclassifications_for_gis.csv'
    shp_export = zero_misclass[['longitude', 'latitude', 'date', 'station_id', 'swe']].copy()
    shp_export.columns = ['lon', 'lat', 'date', 'station_id', 'swe_mm']
    shp_export.to_csv(shp_csv_path, index=False, encoding='utf-8-sig')
    print(f"  GIS-ready CSV saved to: {shp_csv_path}")
    
    return zero_misclass


def plot_all(valid_df, metrics, valid_no_zero_df, metrics_no_zero, zero_df=None):
    """生成所有图表"""
    print(f"\nGenerating plots...")
    ...
    # 1. 原始散点图（包含0值误判点，并用不同颜色标注）
    fig, ax = plt.subplots(figsize=(12, 10))
    
    # 正常点（产品值 > 0）
    normal_mask = valid_df['fused_swe'] > 0
    ax.scatter(station_swe[normal_mask], fused_swe[normal_mask], 
               alpha=0.5, s=20, c='steelblue', edgecolors='none', label='Normal (FusedSWE > 0)')
    
    # 误判点（产品值 == 0 但站点 > 0）
    if zero_df is not None and len(zero_df) > 0:
        zero_station_swe = zero_df['swe'].values
        ax.scatter(zero_station_swe, np.zeros_like(zero_station_swe), 
                   alpha=0.8, s=50, c='red', marker='x', linewidths=2, 
                   label=f'Misclassified (FusedSWE=0, n={len(zero_df)})')
    
    min_val = min(station_swe.min(), fused_swe.min())
    max_val = max(station_swe.max(), fused_swe.max())
    ax.plot([min_val, max_val], [min_val, max_val], 'r--', linewidth=2, label='1:1 Line')
    
    z = np.polyfit(station_swe[normal_mask], fused_swe[normal_mask], 1)
    ...
    ax.set_title('Station SWE vs FusedSWE (Misclassified points highlighted)', fontsize=16, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / 'scatter_plot_with_misclassified.png', dpi=300, bbox_inches='tight')
    plt.close()
    print(f"  ✓ Scatter plot with misclassified points highlighted")
...
def main():
    ...
    # 9. Generate plots (pass zero_df for highlighting)
    plot_all(valid_df, metrics, valid_no_zero_df, metrics_no_zero, zero_df if len(zero_df) > 0 else None)
    ...
    # 12. Print misclassified points summary
    if len(zero_df) > 0:
        print(f"\n{'='*70}")
        print(f"Misclassified Points Summary (Station SWE > 0, FusedSWE = 0)")
        print(f"{'='*70}")
        print(f"  Total points: {len(zero_df)}")
        print(f"  Average station SWE: {zero_df['swe'].mean():.2f} mm")
        print(f"  Max station SWE: {zero_df['swe'].max():.2f} mm")
        print(f"  Min station SWE: {zero_df['swe'].min():.2f} mm")
        print(f"\n  Files exported:")
        print(f"    - CSV: {OUTPUT_DIR}/zero_misclassifications.csv")
        print(f"    - TXT: {OUTPUT_DIR}/zero_misclassifications.txt")
        print(f"    - GIS: {OUTPUT_DIR}/zero_misclassifications_for_gis.csv")
