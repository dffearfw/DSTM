            ...
            try:
                ...
                try:
                    with torch.cuda.amp.autocast(enabled=use_amp):
                        ...
                        # ============ 安全地处理 is_zero_mask ============
                        try:
                            if is_zero_mask is not None and is_zero_mask.numel() == targets.numel():
...
class SWETrainer:
    ...
    def build_model(self, load_pretrained=None, freeze_backbone=True, freeze_strategy='fusion_ft', use_residual=False, is_cv_fold=False):
        ...
        try:
            # ============ 直接写死 C_point = 16 ============
            if not is_cv_fold:
                print("【强制设置】C_point = 21")
            self.config["C_point"] = 21
            ...
            if load_pretrained and freeze_backbone:
                ...
                if freeze_strategy == 'last_layer_only':
                    trainable_keywords = ['head', 'regression', 'output', 'fc', 'correction']
                    for name, param in self.model.named_parameters():
            ...
            if load_pretrained:
                ...
                if not is_cv_fold:
                    print(f"  损失函数: AdaptiveWeightedLoss (weight_power=1.8)")
            ...
            if load_pretrained:
                self.scheduler = optim.lr_scheduler.ReduceLROnPlateau(
                    ...
                    verbose=not is_cv_fold, min_lr=1e-7
                )
                if not is_cv_fold:
                    print(f"  微调解度器: ReduceLROnPlateau (patience=10)")
            else:
                self.scheduler = optim.lr_scheduler.ReduceLROnPlateau(
                    self.optimizer, mode="min", factor=0.5, patience=10, verbose=not is_cv_fold
                )
                if not is_cv_fold:
                    print(f"  训练调度器: ReduceLROnPlateau")
    ...
    def validate(self, dataloader=None, is_fine_tune=False):
        ...
        expected_point_dim = self.config.get("C_point", 21)
        print(f"  期望点特征总维度: {expected_point_dim} (含Wide分支产品值)")
    ...
    def train(self, fine_tune_mode=False, is_cv_sub_run=False):
        """主训练循环 - 完整版本"""
        ...
        for epoch in range(epochs):
            ...
            if verbose:
                ...
                # 交叉验证时简化输出
                print(f"    损失: {train_loss:.6f}, Val Loss: {val_metrics['loss']:.6f}, R²: {val_metrics.get('r2', 0):.4f}")
        ...
        if verbose:
            ...
            if hasattr(self, 'val_history_metrics') and len(self.val_history_metrics) > 0:
    ...
    def run_pretrain_spatial_cv(self):
        ...
        for fold_idx, (train_indices, val_indices) in enumerate(all_folds):
            ...
            self._plot_single_fold_curve(fold_num, fold_train_losses, fold_val_losses, fold_val_r2)
            # ====================================================
        ...
        if all_fold_results:
            ...
            best_r2s = [r['best_val_r2'] for r in all_fold_results]

            print(f"\n📊 十折统计结果:")
            print(f"  验证损失 (Loss): {np.mean(best_losses):.6f} ± {np.std(best_losses):.6f}")
            print(f"  验证 R²:         {np.mean(best_r2s):.4f} ± {np.std(best_r2s):.4f}")
            ...
            cv_results = {
                ...
                'summary': {
                    ...
                    'r2_mean': float(np.mean(best_r2s)),
                    'r2_std': float(np.std(best_r2s)),
                }
            }
            ...
            # 绘制箱线图
            self._plot_spatial_cv_boxplot(best_losses, best_r2s)
            # ====================================================
    ...
    def _plot_single_fold_curve(self, fold_num, train_losses, val_losses, val_r2):
        """绘制单折的训练曲线"""
        try:
            import matplotlib.pyplot as plt
    ...
    def _plot_spatial_cv_curves(self, fold_histories):
        ...
        try:
            ...
            for i, history in enumerate(fold_histories):
                ...
                ax2.plot(epochs, history['val_r2'], 'g-', alpha=0.2, linewidth=0.5)

            # 平均 R²
            avg_r2 = []
            for e in range(max_epochs):
                r2_vals = [h['val_r2'][e] for h in fold_histories if e < len(h['val_r2'])]
                if r2_vals:
    ...
    def _plot_spatial_cv_boxplot(self, best_losses, best_r2s):
        """绘制空间网格CV的箱线图"""
        try:
            import matplotlib.pyplot as plt
            ...
            # 右图：R²
            ax2 = axes[1]
            bp2 = ax2.boxplot(best_r2s, patch_artist=True)
            bp2['boxes'][0].set_facecolor('lightgreen')
            ax2.set_ylabel('R²')
            ax2.set_title('空间网格十折 - 验证 R² 分布')
            ax2.grid(True, alpha=0.3)
            ax2.scatter(1, np.mean(best_r2s), color='blue', s=50, zorder=5,
                       label=f'Mean: {np.mean(best_r2s):.4f}')
            ax2.legend()
    ...
    def run_pretrain_cv_workflow(self):
        """
        预训练阶段的十折交叉验证编排器（增强版）
        支持多年份数据训练
        ...
        # ========== 采样参数 ==========
        samples_per_day = self.config.get("pretrain_samples_per_day", 20000)
        ...
        # 🔥 创建缓存目录
        cache_dir = shared_cache_dir  # 直接使用共享缓存目录
        ...
        for fold, (train_idx, val_idx) in enumerate(kf.split(indices)):
            ...
            success = self.build_model(
                load_pretrained=None,      # 预训练不加载外部权重
            ...
            self._plot_single_fold_curve(
                ...
                fold_train_losses, 
                fold_val_losses, 
                [m.get('r2', 0) for m in fold_val_metrics]
            )
            ...
            fold_histories.append({
                'train_losses': fold_train_losses,
                'val_losses': fold_val_losses,
                'val_r2': [m.get('r2', 0) for m in fold_val_metrics]
            })
        ...
        best_r2s = [m['best_val_r2'] for m in all_fold_metrics]
        ...
        print(f"\n  验证 R²:")
        print(f"    均值: {np.mean(best_r2s):.4f}")
        print(f"    标准差: {np.std(best_r2s):.4f}")
        print(f"    最小值: {np.min(best_r2s):.4f}")
        print(f"    最大值: {np.max(best_r2s):.4f}")
        ...
        for m in all_fold_metrics:
            print(f"{m['fold']:<6} {m['best_epoch']:<8} {m['best_val_loss']:<12.6f} {m['best_val_r2']:<10.4f}")
        ...
        cv_results = {
            ...
            'summary': {
                ...
                'r2_mean': float(np.mean(best_r2s)),
                'r2_std': float(np.std(best_r2s)),
                'r2_min': float(np.min(best_r2s)),
                'r2_max': float(np.max(best_r2s)),
            },
            'config': {
                'batch_size': self.config['batch_size'],
        ...
        # 7. 绘制十折箱线图
        self._plot_cv_boxplot(best_losses, best_r2s)
    ...
    def _plot_cv_training_curves(self, fold_histories):
        ...
        try:
            ...
            for i, history in enumerate(fold_histories):
                epochs = range(1, len(history['val_r2']) + 1)
                ax2.plot(epochs, history['val_r2'], 'g-', alpha=0.3, linewidth=0.5)
    ...
    def _plot_cv_boxplot(self, best_losses, best_r2s):
        """绘制十折指标的箱线图"""
        try:
            import matplotlib.pyplot as plt
    ...
    def run_cv_workflow(self, freeze_strategy=None):
        ...
        for fold, (t_idx_in_cv, v_idx_in_cv) in enumerate(kf.split(real_station_indices)):
            ...
            if preds is not None and len(preds) > 0:
                ...
                fold_metrics = {
                    'r2': r2, 'rmse': rmse, 'mae': mae, 'bias': bias, 'r': r, 'n_samples': len(p_denorm)
                }
    ...
    def run_station_full_cv(self):
        ...
        if is_from_scratch:
            ...
            default_paths = [
                ...
                "/root/autodl-tmp/ablation/final_model.pth",
            ]
            pretrained_to_load = None
    ...
    def evaluate_fine_tune(self, model_path=None, use_tta=False, tta_num=8):
        ...
        print(f"  R²: {eval_metrics.get('r2', 'N/A'):.4f}")
    ...
    def _init_new_pointencoder_weights(self, model):
        """为新增加的特征维度初始化权重（支持13维→16维扩展）
        必须在 load_state_dict(..., strict=False) 之后、optimizer 初始化之前调用
        """
        print(f"\n  【初始化 PointEncoder 新增维度权重】13维 → 16维")

        for name, module in model.named_modules():
            # 精准定位：必须是 point_encoder 中的第一个线性层，且输入维度为16
            if "point_encoder" in name and isinstance(module, nn.Linear) and module.in_features == 16:
                print(f"    找到目标层: {name}")
                print(f"      输入维度: {module.in_features}, 输出维度: {module.out_features}")

                # 获取当前权重（加载预训练后，前13维有值，后3维是随机的）
                current_weight = module.weight.data.clone()
                current_bias = module.bias.data.clone()

                # 保存前13维的权重（来自预训练模型）
                pretrained_weight_13d = current_weight[:, :13].clone()
                pretrained_bias = current_bias.clone()
                ...
                # 恢复前13维的预训练权重
                module.weight.data[:, :13] = pretrained_weight_13d
                module.bias.data = pretrained_bias

                # 新增维度说明
                print(f"    ✓ 已保留前13维的预训练权重")
                print(f"    ✓ 后3维（降水特征2维 + 原产品值1维）已重新初始化")

                # 对新增维度使用较小的初始化，避免初期破坏预训练特征
                with torch.no_grad():
                    # 计算预训练权重的平均绝对值，用于确定缩放因子
                    old_weight_mean = pretrained_weight_13d.abs().mean().item()
                    init_scale = old_weight_mean * 0.1 if old_weight_mean > 0 else 0.01

                    # 让新增维度的权重初始值更小，初期影响更平滑
                    # 特征14-15: 降水累积特征，特征16: 原产品值
                    module.weight.data[:, 13:16] = module.weight.data[:, 13:16] * init_scale
                    print(f"    ✓ 新增维度权重已缩放 {init_scale:.4f}，平滑过渡")
    ...
    def _save_fine_tune_evaluation_results(self, eval_results, predictions_denorm=None, targets_denorm=None):
        """保存微调评估结果"""
        print("  保存微调评估结果...")
        ...
        try:
            ...
            with open(txt_path, "w", encoding="utf-8") as f:
                ...
                f.write("-" * 40 + "\n")
                f.write(f"MSE:  {eval_results.get('mse', 'N/A'):.2f}\n")
                f.write(f"RMSE: {eval_results.get('rmse', 'N/A'):.2f}\n")
                f.write(f"MAE:  {eval_results.get('mae', 'N/A'):.2f}\n")
                ...
                if predictions_denorm is not None and targets_denorm is not None:
                    f.write("数据范围:\n")
                    f.write("-" * 40 + "\n")
                    f.write(f"预测值范围: [{predictions_denorm.min():.2f}, {predictions_denorm.max():.2f}] mm\n")
                    f.write(f"真实值范围: [{targets_denorm.min():.2f}, {targets_denorm.max():.2f}] mm\n")
            ...
            print(f"  R:    {eval_results.get('r', 'N/A'):.4f}")
            print(f"  R²:   {eval_results.get('r2', 'N/A'):.4f}")
            print(f"  RMSE: {eval_results.get('rmse', 'N/A'):.2f} mm")
            print(f"  MAE:  {eval_results.get('mae', 'N/A'):.2f} mm")
            print(f"  Bias: {eval_results.get('bias', 'N/A'):.2f} mm")
            print("=" * 60)
    ...
    def _plot_constraint_scatter(self, predictions, targets, is_zero, eval_results):
        ...
        try:
            ...
            ax1.scatter(targets[pos_mask], predictions[pos_mask], 
                       alpha=0.5, s=20, c='blue', label=f'target>0 (n={np.sum(pos_mask)})',
                       edgecolors='none')
            
            # target=0的样本（红色）
            ax1.scatter(targets[zero_mask], predictions[zero_mask],
                       alpha=0.5, s=20, c='red', label=f'target=0 (n={np.sum(zero_mask)})',
            ...
            # 设置图形属性
            ax1.set_xlabel('真实值 (归一化)', fontsize=14, fontweight='bold')
            ...
            if np.sum(zero_mask) > 0:
                ...
                ax2.set_title('target=0样本预测值分布', fontsize=16, fontweight='bold')
                ax2.grid(True, alpha=0.3)
            else:
    ...
    def plot_cv_metrics_boxplot(self, all_fold_metrics, save_name="cv_metrics_boxplot_en.png"):
        ...
        all_fold_metrics: [{'r2': 0.8, 'rmse': 5.2, 'mae': 3.1}, ...]
        """
        import matplotlib.pyplot as plt
        import pandas as pd
